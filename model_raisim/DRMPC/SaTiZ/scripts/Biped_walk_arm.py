...
class Bipedal_hybrid():
    def __init__(self, cfg):
        ...
        self.u_LB = [-self.motor_mt] * 9
        self.u_UB = [self.motor_mt] * 9
        ...
        # * define variable
        self.q = []
        self.q.append([self.opti.variable(11) for _ in range(self.NN[0])])
        self.q.append([self.opti.variable(11) for _ in range(self.NN[1])])
        self.dq = []
        self.dq.append([self.opti.variable(11) for _ in range(self.NN[0])])
        self.dq.append([self.opti.variable(11) for _ in range(self.NN[1])])
        self.ddq = []
        ...
        self.u.append([self.opti.variable(9) for _ in range(self.NN[0])])
        # ! set the last u to be zero at constraint
        self.u.append([self.opti.variable(9) for _ in range(self.NN[1])])
    ...
    def get_posture(q):
        ...
        lax[0] = q[0]
        lax[1] = lx[0] + L[3]*np.sin(q[6])
        lay[0] = q[1]
        lay[1] = ly[0] - L[3]*np.cos(q[6])

        rax[0] = q[0]
        rax[1] = rx[0] + L[3]*np.sin(q[7])
        ray[0] = q[1]
        ray[1] = ry[0] - L[3]*np.cos(q[7])
            ...
            if not is_ref:
                for i in range(2):
                    for j in range(walker.NN[i]):
                        ...
                        init(q[i][j][6], 0.5 if self.FF_flag else -0.5)
                        init(q[i][j][7], 0.5 if self.FF_flag else -0.5)
                        pass
                    pass
                pass
            else:
...
def main():
    ...
    if vis_flag:
        ...
        g_data = gs[1].subgridspec(2, 5, wspace=0.3, hspace=0.33)

        ax_m = fig.add_subplot(gs[0])
        ax_v = [fig.add_subplot(g_data[0, i]) for i in range(5)]
        ax_u = [fig.add_subplot(g_data[1, i]) for i in range(5)]
        ...
        for tt in np.linspace(0, robot.T, num_frame):
            ...
            patch = patches.Rectangle(
                (pos[0][0]-0.1, pos[1][0]-0.025), 0.2, 0.05, alpha=tt/robot.T*0.8+0.2, lw=0, color=cmap(tt/robot.T))
