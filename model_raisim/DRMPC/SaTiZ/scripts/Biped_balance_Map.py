...
class Bipedal_hybrid():
    def __init__(self, Mas, inert, cfg):
        ...
        self.q_LB = [-np.pi/30, -np.pi, -np.pi/30, 0, -np.pi*0.9] 
    ...
    def getConstraints(self, walker):
        ...
        for temp_u in walker.u:
            ceq.extend([walker.opti.bounded(walker.u_LB[j],
                        temp_u[j], walker.u_UB[j]) for j in range(4)])
            pass
        # endregion
        ...
        for j in range(len(walker.u)):
            ceq.extend([walker.u[j][k]-ca.fmax(mt - (walker.dq[j][k+1] -
                                                        cs)/(ms-cs)*mt, 0) <= 0 for k in range(4)])
            ceq.extend([walker.u[j][k]-ca.fmin(-mt + (walker.dq[j][k+1] +
                                                            cs)/(-ms+cs)*mt, 0) >= 0 for k in range(4)])
            pass
        # endregion
        ...
        try:
            ...
            for j in range(robot.N):
                ...
                u.append([sol1.value(robot.u[j][k])
                            for k in range(4)] if j < (robot.N) else [0]*4)
                pass
            pass
        except:
            ...
            for j in range(robot.N):
                ...
                u.append([value(robot.u[j][k])
                            for k in range(4)] if j < (robot.N-1) else [0]*4)
                pass
            pass
        finally:
            q = np.asarray(q)
...
def main(Mass, inertia, armflag):
    ...
    if vis_flag:
        ...
        ax_u[0].plot(t, Fx)
        ax_u[0].plot(t, Fy)
        ax_u[0].set_xlabel('ankle', fontsize = labelfont)
        ax_u[0].set_ylabel('Force(N)', fontsize = labelfonty)
        ax_u[1].plot(t, u[:, 0])
        ax_u[1].set_xlabel('knee', fontsize = labelfont)
        ax_u[2].plot(t, u[:, 1])
        ax_u[2].set_xlabel('waist', fontsize = labelfont)
        ax_u[3].plot(t, u[:, 2])
        ax_u[3].set_xlabel('shoulder', fontsize = labelfont)
        ax_u[4].plot(t, u[:, 3])
        ax_u[4].set_xlabel('elbow', fontsize = labelfont)
...
def ForceMapMV():
    ...
    I_arm = [0.03, 0.05, 0.07]
    I_label = list(map(str, I_arm))

    Mass = [6.8, 20, 30, 4]
    inertia = [0.14, 0.3, 0.625, 0.03]
    ...
    if saveflag:
        ...
        Data = {'Fy': Fy, 'u_h': u_h, "u_k": u_k, "u_s": u_s, "u_e": u_e}
    ...
    pcm1 = ax1.imshow(Fy, vmin = 300, vmax = 1200)
